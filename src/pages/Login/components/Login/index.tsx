...
export default function Login() {
  ...
  const [verifyImage, setImage] = useState('');
  ...
  const onSubmit = async (e: SubmitContext) => {
    if (e.validateResult === true) {
      try {
        ...
        await dispatch(login({ ...formValue, uuid }));
      ...
      >
        {loginType === 'password' && (
          <>
            ...
            <FormItem name='code' rules={[{ required: true, message: '验证码', type: 'error' }]}>
              <Input size='large' placeholder='请输入验证码' prefixIcon={<UserIcon />}></Input>
              <img onClick={getCaptchaCode} src={`data:image/png;base64,${verifyImage}`} />
            </FormItem>
